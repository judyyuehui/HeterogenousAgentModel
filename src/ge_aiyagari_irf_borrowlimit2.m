...
plot_path = ['/Users/judyyue/Dropbox/course_material/2024_winter/HeterogeneousHouseholds/FinalProject/graphs/asset_quality_'];
addpath(plot_path);

%% OPTIONS
options.Display                 = 1;
options.MakePlotsSteadyState    = 1;
...
%preference shock
param.preferenceshock_size    = 0; %-0.001; %shock to preference
param.preferenceshock_decay   = 0.5; % quarterly decay rate


%asset quality shock
param.aqshock_size    = 0.001; %0.001; %shock to asset quality
param.aqshock_decay   = 0.5; % quarterly decay rate
...
shocks.aqshock = param.aqshock_size.*exp(-param.aqshock_decay.*(grids.Tgrid-param.dt_initial));
...
if options.MakePlotsSteadyState ==1 
    PlotResultsSteadyState(1,param,grids,steadystate, plot_path)
end
...
function [res,steadystate] = market_clearing_steadystate(r,param,grids)
    ...
    %asset supply
    gamarg = sum(gmat,2);
    Ea = assetquality*sum(gamarg.*agrid.*adelta);
...
function [res,irf] = market_clearing_irf(x,param,grids,steadystate,shocks)
    ...
    irf.aq = steadystate.aq + shocks.aqshock;
...
function [Vnew,A,con,sav] = UpdateHJB(V,param,grids,passedvars)
    ...
    savf = r.*aagrid + aq*wage.*yscale.*yygrid - aq*conf;
    Hf = pref*param.u(conf) + dVf.*savf;
    
    %consumption and savings with backward difference
    conb = pref_inv*param.u1inv(max(dVb,param.mindV));
    savb = r.*aagrid + aq*wage.*yscale.*yygrid - aq*conb;
...
%%
function PlotResultsSteadyState(fignum,param,grids,equm, plot_path)
    ...
    saveas(gcf, [plot_path, 'plot_ss.png']);
...
function PlotResultsIRF(fignum,param,grids,steadystate,irf, plot_path)
    ...
    plot(Tgrid,irf.aq .* irf.Ea,'r--','LineWidth',1);
